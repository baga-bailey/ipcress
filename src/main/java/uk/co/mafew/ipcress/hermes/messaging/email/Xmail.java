package uk.co.mafew.ipcress.hermes.messaging.email;
		...
		if (boundary.compareTo("") != 0) {
			try {
				...
				for (int i = 0; i < ((Multipart) content).getCount(); i++) {
					...
					childBODYElem.appendChild(doc
							.createTextNode(getBody((messagePart)).replaceAll("&", "&amp;").replaceAll("<", "&lt;")));
			...
			partsElem.appendChild(MIMEElem);
			childBODYElem = doc.createElement("BODY");
			childBODYElem.appendChild(
					doc.createTextNode(getBody((messagePart)).replaceAll("&", "&amp;").replaceAll("<", "&lt;")));
			MIMEElem.appendChild(childBODYElem);
			BODYElem.appendChild(partsElem);
			doc.getDocumentElement().appendChild(BODYElem);
		}
		...
		try {
			...
			while (headers.hasMoreElements()) {
				...
				if (header.getName().compareTo("Content-Type") == 0
						|| header.getName().compareTo("Content-Disposition") == 0) {
				...
				// System.out.println(convertToString(doc));
				if (header.getName().compareTo("From") == 0 || header.getName().compareTo("To") == 0
						|| header.getName().compareTo("Subject") == 0) {

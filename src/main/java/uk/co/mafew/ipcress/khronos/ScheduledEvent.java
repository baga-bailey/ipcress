...
import uk.co.mafew.ipcress.apollo.file.*;
import uk.co.mafew.ipcress.hephaestus.*;
import uk.co.mafew.ipcress.apollo.logging.*;
import uk.co.mafew.ipcress.narcissus.*;
	...
	public void start(Element evt) {
		...
		try {
			...
			for (int i = 0; i < list.getLength(); i++) {
				...
				String nodevalue = elem.getElementsByTagName("hour").item(0).getFirstChild().getNodeValue();
				...
				nodevalue = elem.getElementsByTagName("minute").item(0).getFirstChild().getNodeValue();
				if (nodevalue.compareToIgnoreCase("now") == 0) {
				...
				nodevalue = elem.getElementsByTagName("second").item(0).getFirstChild().getNodeValue();
				if (nodevalue.compareToIgnoreCase("now") == 0) {
				...
				nodevalue = elem.getElementsByTagName("day").item(0).getFirstChild().getNodeValue();
				if (nodevalue.compareToIgnoreCase("now") == 0) {
				...
				nodevalue = elem.getElementsByTagName("month").item(0).getFirstChild().getNodeValue();
				if (nodevalue.compareToIgnoreCase("now") == 0) {
				...
				nodevalue = elem.getElementsByTagName("year").item(0).getFirstChild().getNodeValue();
				if (nodevalue.compareToIgnoreCase("now") == 0) {
				...
				int delay = (new Integer(elem.getElementsByTagName("delay").item(0).getFirstChild().getNodeValue()))
						.intValue();
				String period = elem.getElementsByTagName("period").item(0).getFirstChild().getNodeValue();
				it.addEvent(new RegularEvent(hourOfDay, minute, second, day, month, year, delay, period));
			}
			...
			if (list.getLength() == 1) {
				...
				NodeList groupList = elem.getElementsByTagName("ConditionGroup");
				for (int j = 0; j < groupList.getLength(); j++) {
					...
					} else if (groupElem.getAttribute("operator").compareTo("or") == 0) {
						// Add to the Iterator 'orList'
						it.addOrGroup(conGroup);
					}

					NodeList conditionList = groupElem.getElementsByTagName("Condition");
					for (int x = 0; x < conditionList.getLength(); x++) {
						...
						if (conditionElem.getAttribute("operator").compareTo("and") == 0)
							conGroup.addAndCondition(cw);
						if (conditionElem.getAttribute("operator").compareTo("or") == 0)
		...
		Element targetClass = (Element) event.getElementsByTagName("TargetClass").item(0);
		...
		if (PERSISTENCE.compareToIgnoreCase("database") == 0) {
			...
			try {
				String target = (String) xPath.compile("//ScheduledEvent/TargetClass/method/params/param/value/text()")
						.evaluate(event.getOwnerDocument(), XPathConstants.STRING);
				if (target.compareTo("uk.co.mafew.hephaestus.StartProcess") == 0) {
					target = (String) xPath.compile("/ScheduledEvent/TargetClass/name/text()").evaluate(event,
